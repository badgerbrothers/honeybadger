...
Manus MVP 是一个 AI 驱动的任务执行平台，使用户能够将复杂的多步骤工作流委托给自主代理。与传统聊天机器人不同，Manus 为每个任务创建隔离的执行环境（沙箱），允许代理使用真实工具——浏览器、代码解释器、文件系统——来完成目标，如生成研究报告、构建网页或分析文档。
...
如果用户可以完成此端到端工作流，则 MVP 成功：

1. 创建项目
2. 在该项目中开始对话
3. 请求复杂任务（例如，"研究特斯拉第四季度财报并创建报告"）
4. 查看代理执行多个步骤并实时更新
5. 查看生成的工件（报告、代码等）
6. 将工件保存到项目目录
7. 发起引用先前工作的后续任务
...
- **原始需求：** `docs/manus-mvp-requirements.md` - 详细的中文需求文档
- **架构图：** 将在阶段 1 创建
